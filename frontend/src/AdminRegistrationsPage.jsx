...
const downloadInternalParticipantsExcel = async ({ apiBaseUrl, adminToken, onError }) => {
  try {
    if (!adminToken) {
      throw new Error('Admin API token missing. Login with backend admin credentials.')
    }

    const response = await fetch(`${apiBaseUrl}/internal-participants/export/excel`, {
      headers: {
        Authorization: `Bearer ${adminToken}`,
      },
    })
    ...
    if (!response.ok) {
      throw new Error('Failed to download internal participants file')
    }

    const blob = await response.blob()
    const contentDisposition = response.headers.get('content-disposition') || ''
    const match = contentDisposition.match(/filename="?([^"]+)"?/i)
    const fileName = match?.[1] || 'internal-participants.xlsx'
    ...
    onError?.(error?.message || 'Failed to download internal participants file')
  }
}

const downloadExternalParticipantsExcel = async ({ apiBaseUrl, adminToken, onError }) => {
  try {
    if (!adminToken) {
      throw new Error('Admin API token missing. Login with backend admin credentials.')
    }

    const response = await fetch(`${apiBaseUrl}/external-participants/export/excel`, {
      headers: {
        Authorization: `Bearer ${adminToken}`,
      },
    })
    ...
    if (!response.ok) {
      throw new Error('Failed to download external participants file')
    }

    const blob = await response.blob()
    const contentDisposition = response.headers.get('content-disposition') || ''
    const match = contentDisposition.match(/filename="?([^"]+)"?/i)
    const fileName = match?.[1] || 'external-participants.xlsx'
    ...
    onError?.(error?.message || 'Failed to download external participants file')
  }
}
...
const fetchInternalParticipants = async ({ apiBaseUrl, adminToken }) => {
  if (!adminToken) {
    throw new Error('Admin API token missing. Login with backend admin credentials.')
  }

  const response = await fetch(`${apiBaseUrl}/internal-participants`, {
    headers: {
      Authorization: `Bearer ${adminToken}`,
    },
  })
  ...
  if (!response.ok) {
    throw new Error('Failed to fetch internal participant details')
  }
...
const fetchExternalParticipants = async ({ apiBaseUrl, adminToken }) => {
  if (!adminToken) {
    throw new Error('Admin API token missing. Login with backend admin credentials.')
  }

  const response = await fetch(`${apiBaseUrl}/external-participants`, {
    headers: {
      Authorization: `Bearer ${adminToken}`,
    },
  })
  ...
  if (!response.ok) {
    throw new Error('Failed to fetch external participant details')
  }
    ...
    () => [
      ...
      },
      {
        ...
        onRefresh: async () => {
          ...
          setErrorInternal('')
          try {
            const data = await fetchInternalParticipants({ apiBaseUrl, adminToken })
        ...
        onDownload: async () => {
          ...
          setErrorInternal('')
          await downloadInternalParticipantsExcel({ apiBaseUrl, adminToken, onError: setErrorInternal })
      ...
      },
      {
        ...
        onRefresh: async () => {
          ...
          setErrorExternal('')
          try {
            const data = await fetchExternalParticipants({ apiBaseUrl, adminToken })
        ...
        onDownload: async () => {
          ...
          setErrorExternal('')
          await downloadExternalParticipantsExcel({ apiBaseUrl, adminToken, onError: setErrorExternal })
                  ...
                  <table className="admin-table">
                    <thead>
                      <tr>
                        ...
                        <th>Email Id</th>
                        <th>Contact No (Office & Mobile)</th>
                      </tr>
                    </thead>
                    <tbody>
                      {panel.rows.map((item) => (
