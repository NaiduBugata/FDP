...
const DEFAULT_XLSX_PATH = path.resolve(__dirname, '..', '..', 'FDP External Participation details.xlsx')
const DEFAULT_SHEET = null
const DEFAULT_COLLECTION = 'external_participants'
...
const main = async () => {
	...
	for (let rowNumber = headerRowNumber + 1; rowNumber <= worksheet.rowCount; rowNumber += 1) {
		...
		const hasAnyValue =
			row.values && row.values.some((v, i) => i !== 0 && v != null && String(v).trim() !== '')
		if (!hasAnyValue) {
			continue
		}
		...
		const serialNoText = valuesText[0] || ''
		const nameText = valuesText[1] || ''
		...
		const filter = {
			sourceFile,
			sheetName,
			...(nameText ? { name: nameText, serialNo: serialNoText } : { rowNumber }),
		}
